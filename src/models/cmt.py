...
class LightMutilHeadSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.relative_pos_embeeding:
            attn = attn + self.position_embeeding[self.relative_indices[:, :, 0], self.relative_indices[:, :, 1]][:,
                          :k_n]
        elif self.no_distance_pos_embeeding:
...
class CMTStem(nn.Module):
    ...
    def __init__(self, kernel_size, in_channel, out_channel, layers_num):
        ...
        self.conv3x3_gelu_bn_list = nn.ModuleList(
            [ConvGeluBN(kernel_size=kernel_size, in_channel=out_channel, out_channel=out_channel, stride_size=1) for _
...
class ConvolutionMeetVisionTransformers(nn.Module):
    def __init__(self,
        ...
        self.stage1 = CMTBlock(
            dim=dims_list[1], num_heads=heads_list[0], relative_pos_embeeding=self.rpe,
            no_distance_pos_embeeding=self.pe_nd,
            features_size=resolution_list[1], qkv_bias=self.qkv_bias, attn_drop=self.attn_drop,
            ffn_ratio=self.ffn_ratio,
            proj_drop=self.proj_drop, sr_ratio=sr_ratio_list[0], num_layers=block_list[0],
            drop_path_rate=dpr[:block_list[0]])
        self.stage2 = CMTBlock(
            dim=dims_list[2], num_heads=heads_list[1], relative_pos_embeeding=self.rpe,
            no_distance_pos_embeeding=self.pe_nd,
            features_size=resolution_list[2], qkv_bias=self.qkv_bias, attn_drop=self.attn_drop,
            ffn_ratio=self.ffn_ratio,
            proj_drop=self.proj_drop, sr_ratio=sr_ratio_list[1], num_layers=block_list[1],
            drop_path_rate=dpr[sum(block_list[:1]): sum(block_list[:2])])
        self.stage3 = CMTBlock(
            dim=dims_list[3], num_heads=heads_list[2], relative_pos_embeeding=self.rpe,
            no_distance_pos_embeeding=self.pe_nd,
            features_size=resolution_list[3], qkv_bias=self.qkv_bias, attn_drop=self.attn_drop,
            ffn_ratio=self.ffn_ratio,
            proj_drop=self.proj_drop, sr_ratio=sr_ratio_list[2], num_layers=block_list[2],
            drop_path_rate=dpr[sum(block_list[:2]): sum(block_list[:3])])
        self.stage4 = CMTBlock(
            dim=dims_list[4], num_heads=heads_list[3], relative_pos_embeeding=self.rpe,
            no_distance_pos_embeeding=self.pe_nd,
            features_size=resolution_list[4], qkv_bias=self.qkv_bias, attn_drop=self.attn_drop,
            ffn_ratio=self.ffn_ratio,
            proj_drop=self.proj_drop, sr_ratio=sr_ratio_list[3], num_layers=block_list[3],
            drop_path_rate=dpr[sum(block_list[:3]): sum(block_list[:4])])
