...
class Deltar(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        modules =[self.depth_head, self.decoder, self.conv_out,self.hist_encoder]
        for s in modules:
            for m in s.modules():
    ...
    def forward(self, input_data, **kwargs):
        ...
        if self.training:
            ...
            return bin_edges, pred, prob, None
    def get_1x_lr_params(self):  # lr/10 learning rate
        if args.hist_encoder_10x:
            modules = [self.img_encoder]
        else:
            modules = [self.img_encoder, self.hist_encoder]
        for m in modules:
            yield from m.parameters()

    def get_10x_lr_params(self):  # lr learning rate
        if args.hist_encoder_10x:
            modules = [self.decoder, self.depth_head, self.conv_out,self.hist_encoder]
        else:
