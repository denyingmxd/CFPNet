...
class CompletionEncoder(nn.Module):
    ...
    def forward(
        ...
        x = input_image
        x = F.interpolate(x, size=disps.shape[2:], mode='bilinear', align_corners=True)
        x = torch.cat([x, disps], 1)
        x = self.conv1(x)
        x = self.bn1(x)
...
class CompletionDecoder(nn.Module):
    ...
    def forward(self, input_features: List[torch.Tensor]):
        ...
        for i in range(4, -1, -1):
            ...
            if self.use_skips and i > 0:
                if x[0].shape[2:]!=input_features[i - 1].shape[2:]:
                    x += [F.interpolate(input_features[i - 1], size=x[0].shape[2:], mode='bilinear', align_corners=True)]
                else:
        ...
        return 10*self.sigmoid(outputs[("depth", 0)])


class DepthCompletion2(nn.Module):
    ...
    def __init__(
        ...
        super(DepthCompletion2, self).__init__()
